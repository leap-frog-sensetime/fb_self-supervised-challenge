...
def train_svm(opts):
    ...
    for cls_idx in range(len(cls_list)):
        ...
        for cost_idx in range(len(costs_list)):
                ...
                fwrite = open(out_file, 'wb')
                pickle.dump(clf, fwrite)
                
                #joblib.dump(clf, 'tmp/Rotation_voc07_conv3_model/{}_test.joblib'.format(cls))
                with open(out_file, 'rb') as fwrite:
                    new_svm1 = pickle.load(fwrite)
                print(new_svm1)    
