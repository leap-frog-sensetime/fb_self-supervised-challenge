...
def get_data_transformations(rotation=0):
    ...
    final_process_train = [transforms.ToTensor(), Drop_part(24), tr_normalize]
...
class Drop_part(object):
    def __init__(self, size):
        self.dropsize = size
    def __call__(self, img):
        return drop_img(img, self.dropsize)

def drop_img(img,dropsize):
    #begin = int((224-int(dropsize))/2)
    #img[:,begin:begin+dropsize,begin:begin+dropsize] = 0
    large_size  = int(224-dropsize-1)
    for i in range(2):
        begin_w = random.randint(0,large_size)
        begin_h = random.randint(0,large_size)
        img[:,begin_w:begin_w+dropsize,begin_h:begin_h+dropsize] = 0
    return img
